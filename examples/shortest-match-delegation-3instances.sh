...
#   5. Submit a job with --dependency=delegate:shortest_match to trigger selection.
...
CONFIG_PATH="${SCRIPT_DIR}/shortest-match-clusters.toml"
...
printf '=== Shortest-Match Delegation Example (3 targets) ===\n\n'
...
# Step 4: Create load on target-0 by submitting two jobs with assign:0 policy.
# These jobs will be pending when we later query shortest_match, making
# target-0 appear as the more loaded option.
...
# Step 5: Submit a job with shortest_match policy.
# Because target-0 has 2 pending jobs while targets 1 and 2 are idle,
# shortest_match should select one of the idle targets.
printf '[5/5] Submitting job with --dependency=delegate:shortest_match ...\n'
if submit_output="$(flux proxy "${SOURCE_INSTANCE}" flux submit --dependency='delegate:shortest_match' -N1 -n1 hostname 2>&1)"; then
...
if printf '%s\n' "${EVENTLOG}" | grep -q 'clean$'; then
    printf '\nPASS: Job was delegated via shortest_match and finished cleanly.\n'
else
