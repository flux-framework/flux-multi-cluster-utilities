...
CONFIG_FILE="${SCRIPT_DIR}/shortest-match-clusters.toml"
...
# --- Step 4: Create load on target-0 ---------------------------------------
...
# --- Step 5: Submit a job with shortest_match delegation -------------------

printf '[5/5] Submitting job with --dependency=delegate:shortest_match ...\n'
JOB_ID="$(flux proxy "${SOURCE_INSTANCE}" flux submit --dependency=delegate:shortest_match -t 10m -N1 -n1 hostname | tail -n 1 | tr -d '[:space:]')"
