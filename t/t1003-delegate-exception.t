...
test_expect_success 'alloc exception on B re-delegates to C and completes' '
	jobid=$(flux submit --dependency=delegate:assign:0 sleep 3) &&
...
test_expect_success 'alloc exceptions on B then C re-delegate to D and completes' '
	jobid=$(flux submit --dependency=delegate:assign:0 sleep 3) &&
	wait_for_event_count "${jobid}" "delegate::submit" 1 &&
	delegated_b=$(get_last_delegate_id "${jobid}") &&
	check_job_visible "${uri_b}" "${delegated_b}" &&
...
test_expect_success 'alloc exceptions on B C and D fail back in source cluster' '
	...
	flux job wait-event --timeout=60 "${jobid}" clean &&
	flux job eventlog "${jobid}" | grep -Eq "failed|exception"
'
