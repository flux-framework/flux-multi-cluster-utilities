...
static int load_config (struct cluster_config *config, const char *config_path)
{
    ...
    if (!(root = json_load_file (config_path, 0, &error))) {
        flux_log (config->h,
                  LOG_ERR,
                  "Failed to load selection config %s: %s (line %d)",
                  config_path,
                  error.text,
                  error.line);
        errno = EINVAL;
    ...
    if (json_unpack (root, "{s:o}", "clusters", &clusters) < 0 || !json_is_array (clusters)) {
        flux_log (config->h,
                  LOG_ERR,
                  "Selection config %s is missing a string array named 'clusters'",
                  config_path);
        json_decref (root);
        errno = EINVAL;
        return -1;
    }
    count = json_array_size (clusters);
    if (count == 0) {
    ...
    if (!(config->uris = calloc (count, sizeof (*config->uris)))) {
        json_decref (root);
        return -1;
    }
    for (index = 0; index < count; index++) {
        json_t *entry = json_array_get (clusters, index);
        const char *uri;

        if (!json_is_string (entry) || !(uri = json_string_value (entry)) || *uri == '\0') {
            flux_log (config->h,
                      LOG_ERR,
                      "Selection config %s has invalid cluster entry at index %zu",
                      config_path,
                      index);
            free_uris (config);
            json_decref (root);
